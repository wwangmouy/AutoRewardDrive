...
class FDPF():
    ...
    def getIntensityAt(self, ego_s, ego_l, ego_delta_yaw, ego_v):
        ...
        # Lane boundaries for single-lane per direction (e.g., Town02)
        # 0.5 * lane_width covers exactly one lane (left edge to right edge)
        # For dual-lane roads: each direction has 1 lane
        self.left_boundry = self.lane_width * 0.5 - self.margin_distance
        self.right_boundry = -self.lane_width * 0.5 + self.margin_distance

        left_off_road_distance =  max(0, ego_l - self.left_boundry)
        right_off_road_distance =  min(0, ego_l - self.right_boundry)
        
        # Asymmetric boundary penalties for Town02 dual-lane roads:
        # - Centerline (left): moderate risk (crossing into oncoming traffic)
        # - Road shoulder (right): high risk (hitting guardrail/ditch)
        centerline_weight = 1.0      # Baseline penalty for crossing centerline
        road_shoulder_weight = 2.0   # 2x penalty for going off right edge
        
        intensity_left_bound = centerline_weight * left_off_road_distance**2
        intensity_right_bound = road_shoulder_weight * right_off_road_distance**2
